...
interface Tab { id: string; icon: string; label: string; sub: string }
...
async function save() {
  ...
  if (active.value === 'projects') {
    for (const proj of projects.value) {
      ...
      await upsertProject({
        id: rest.id, name: rest.name, description: rest.description || null,
        emoji: rest.emoji || '🚀',
        tags: tagsInput?.split(',').map((t: string) => t.trim()).filter(Boolean) || [],
        demo_url: rest.demo_url || null, github_url: rest.github_url || null,
        visible: rest.visible ?? true, order_index: rest.order_index ?? 0,
      })
    }
  }
...
async function uploadPhoto(event: Event) {
  ...
  if (error) { uploading.value = false; return }
...
// Projects
function addProject() {
  projects.value.push({ name: '', description: '', emoji: '🚀', tags: [], tagsInput: '', demo_url: '', github_url: '', visible: true, order_index: projects.value.length })
}
...
async function handleChangePassword(newPw: string, confirmPw: string) {
  if (newPw !== confirmPw) { pwMsg.value = { ok: false, text: 'As senhas não coincidem.' }; return }
